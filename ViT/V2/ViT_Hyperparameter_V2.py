...
TRAIN_JSON = "/cfs/earth/scratch/benkehel/ViLT/Data/trainV2_ViLT.json"
VAL_JSON = "/cfs/earth/scratch/benkehel/ViLT/Data/valV2_ViLT.json"
LOG_DIR = "/cfs/earth/scratch/benkehel/ViT/V2/optuna_logsV2"
...
label_to_index = {label: idx for idx, label in enumerate(material_labels)}
index_to_label = {idx: label for label, idx in label_to_index.items()}

def extract_disposal_from_gpt(text):
    labels = [0] * len(material_labels)
...
def objective(trial):
    ...
    model = ViTMultiLabel(base_model, len(material_labels)).to(device)
